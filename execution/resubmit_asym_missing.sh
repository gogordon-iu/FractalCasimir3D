...
JOBS=""
echo "Checking for missing segment cache files for Asymmetric Sweep..."

for config in "both" "self"; do
    for seg in {0..17}; do
        m_start=$((seg * 6))
        m_end=$(((seg + 1) * 6))
        cache_file=".tmp/meep_d_0.2500_N_3_Nbot_2_Phosphorene_tuned_res_40_theta_90.0_eps_2.1_L_2.00_config_${config}_moments_${m_start}_${m_end}.json"
...
if [ -n "$JOBS" ]; then
    ...
    PLOT_JOB_ID=$(sbatch --dependency=afterany:${JOBS} --parsable "execution/submit_asym_L_${L}_d_${d}_plot.sbatch")
    echo "Submitted plot job -> Job ID: ${PLOT_JOB_ID}"
else
    echo "--------------------------------------------------"
    echo "All 36 segment files are present! Running plot compilation directly..."
    sbatch "execution/submit_asym_L_${L}_d_${d}_plot.sbatch"
fi
echo "=================================================="
