#!/usr/bin/env python3
"""
Results Analyzer for Rotary Vacuum Casimir Clutch
--------------------------------------------------------------------------------
Processes simulation JSON outputs from results_rotary_clutch/ and evaluates:
...
2. Identifies the critical clutch disengagement/engagement angle theta_clutch
   where the force switches from repulsion (F_z > 0) to attraction (F_z < 0).
...
def main():
    res_dir = os.path.join(REPO_ROOT, "results_rotary_clutch")
    ...
    print("=" * 95)
    print("ROTARY VACUUM CASIMIR CLUTCH ANALYSIS (Levin-Johnson Rotary Mechanism)")
    print("=" * 95)
    print(f"{'Task ID':<10}{'theta (deg)':<14}{'F_total (fN)':<18}{'Pressure (Pa)':<18}{'Regime':<20}")
    print("-" * 95)
    ...
    for d in data:
        ...
        print(f"{tid:<10}{th:<14.1f}{f_net_fN:<+18.4f}{p:<+18.4e}{regime_str:<20}")
    ...
    if has_repulsive and has_attractive:
        print(">>> SUCCESS: Full Rotary Casimir Clutch verified! Transitions between Repulsion and Attraction.")
        if th_clutch is not None:
            print(f">>> ROTARY CLUTCH THRESHOLD: theta_clutch = {th_clutch:.2f} deg")
    elif has_repulsive:
        print("Notice: Repulsive forces detected; run additional angles up to 90 deg to capture attractive engagement.")
    else:
    ...
    # 1. Save summary JSON
    summary_data = {
        "architecture": "rotary_vacuum_casimir_clutch",
    ...
    summary_path = os.path.join(res_dir, "rotary_clutch_summary.json")
    with open(summary_path, "w") as f_sum:
        json.dump(summary_data, f_sum, indent=4)
    print(f"Summary JSON saved: {summary_path}")

    # 2. Save LaTeX Table
    table_path = os.path.join(res_dir, "table_rotary_clutch.tex")
    with open(table_path, "w") as f_tex:
        f_tex.write(r"\begin{table}[htbp]" + "\n")
        f_tex.write(r"\centering" + "\n")
        f_tex.write(r"\caption{Rotary Vacuum Casimir Clutch Force and Pressure vs Rotation Angle $\theta$ ($z_{\text{tip}} = 15\,\text{nm}$, $r_0 = 350\,\text{nm}$, $W = 350\,\text{nm}$).}" + "\n")
        f_tex.write(r"\label{tab:rotary_casimir_clutch}" + "\n")
        f_tex.write(r"\begin{tabular}{ccccc}" + "\n")
        f_tex.write(r"\hline" + "\n")
        f_tex.write(r"Task & $\theta$ (deg) & $F_{\text{total}}$ (fN) & Pressure (Pa) & Clutch State \\" + "\n")
        f_tex.write(r"\hline" + "\n")
        for d in data:
            tid = d.get("task_id", 0)
            th = d.get("theta_deg", 0.0)
            f_net_fN = d.get("force_total_fN", d.get("force_total_meep", 0.0) * 31.615)
            p = d.get("pressure_Pa", 0.0)
            state = "Disengaged (Repulsive)" if f_net_fN > 0 else "Engaged (Attractive)"
            f_tex.write(f"{tid} & {th:.1f} & {f_net_fN:+.2f} & {p:+.2e} & {state} \\\\\n")
        f_tex.write(r"\hline" + "\n")
    ...
    try:
        ...
        # (a) Full-Range Rotor Force F_z(theta)
        ax1.axhline(0, color="k", linestyle="--", linewidth=0.8, alpha=0.7)
        if th_clutch is not None:
            ax1.axvline(th_clutch, color="crimson", linestyle="-.", linewidth=0.8, label=f"$\\theta_c = {th_clutch:.1f}^\\circ$")
        ax1.plot(th_arr, f_arr, "o-", color="#1f77b4", linewidth=1.5, markersize=5, label=r"$F_z(\theta)$")
        ax1.fill_between(th_arr, 0, f_arr, where=(f_arr > 0), color="green", alpha=0.18, label="Repulsive (Levitating)")
        ax1.fill_between(th_arr, 0, f_arr, where=(f_arr < 0), color="blue", alpha=0.10, label="Attractive (Clamping)")
        ax1.set_xlabel(r"Rotation Angle $\theta$ (deg)")
        ax1.set_ylabel(r"Total Rotor Force $F_z$ (fN)")
        ax1.set_title(r"(a) Full Clutch Characteristic")
        ...
        ax2.set_ylabel(r"Total Rotor Force $F_z$ (fN)")
        ...
        ax3.set_title(r"(c) Rotor Face Pressure $P(\theta)$")
        ax3.grid(True, linestyle=":", alpha=0.5)
        ax3.legend(loc="lower left", frameon=True)

        plt.tight_layout()
        fig_path = os.path.join(res_dir, "fig_rotary_casimir_clutch.png")
