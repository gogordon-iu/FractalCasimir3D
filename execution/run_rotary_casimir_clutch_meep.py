#!/usr/bin/env python3
"""
Rotary Vacuum Casimir Clutch Simulation Engine
--------------------------------------------------------------------------------
Evaluates the normal Casimir force F_z between a two-needle rotor and a thin
conducting stator membrane with dual circular apertures as a function of the
relative rotation angle theta.

Physical Mechanism:
At theta = 0 deg: Both needles align coaxially above the circular apertures,
entering the Levin-Johnson near-field regime (z_tip < W / 4). Transverse field
line expulsion drives a strictly positive, repulsive vacuum Casimir force (F_z > 0),
providing passive non-contact levitation.

At theta = 90 deg: Both needles rotate over the unbroken, solid metallic membrane
(d > 300 nm from any aperture rim). Standard boundary conditions apply, yielding
strong attractive Casimir adhesion (F_z << 0), providing mechanical clamping.

By modulating theta alone at constant vertical clearance z_tip, the interaction
transitions continuously from repulsive levitation to attractive engagement.
...
def run_rotary_casimir_clutch_simulation(
    theta_deg=0.0,          # Relative rotation angle (degrees)
    r_rotor=0.35,           # Rotor and aperture radial placement (microns)
    W_aperture=0.35,        # Aperture hole diameter (microns)
    ...
    resolution=80,          # Yee grid resolution (pixels per micron)
    ...
    L_plate=1.8,            # Stator membrane lateral span (microns)
    chk_tag="rotary_clutch",# Checkpoint file identifier
    max_walltime_hours=7.5, # Walltime budget limit
    no_cache=False
):
    """
    Executes 3D FDTD Maxwell stress tensor integration for the rotary Casimir clutch.
    Integrates stress tensor around Needle 1. Total rotor force is 2 * F_1,z by C2 symmetry.
    """
    if mp is None:
    ...
    os.makedirs("results_rotary_clutch", exist_ok=True)

    # 1. Coordinate and Integration Box Setup
    # Coordinate origin z = 0 is the top surface of the stator membrane.
    ...
    # Standoffs:
    # Lateral: at least 2 Yee grid cells (2 * dx)
    delta_xy = max(0.025, 2.0 * dx)
    delta_z_top = max(0.025, 2.0 * dx)

    # Bottom face of stress tensor integration box:
    # Placed at z_bot = z_tip / 2.0, strictly within the vacuum gap (0 < z_bot < z_tip).
    # This guarantees the box never intersects the plate (z <= 0) or the needle (z >= z_tip)
    # at any rotation angle theta, avoiding unphysical boundary-slicing artifacts.
    z_bot = z_tip / 2.0
    ...
    # 6 sides of bounding box S_1 enclosing Needle 1 centered at (+r_rotor, 0)
    sides_info = [
        {"center": mp.Vector3(r_rotor - sx_box / 2.0, 0.0, z_box_center), "size": mp.Vector3(0.0, sy_box, sz_box), "orientation": -1.0},
        {"center": mp.Vector3(r_rotor + sx_box / 2.0, 0.0, z_box_center), "size": mp.Vector3(0.0, sy_box, sz_box), "orientation": +1.0},
        {"center": mp.Vector3(r_rotor, -sy_box / 2.0, z_box_center), "size": mp.Vector3(sx_box, 0.0, sz_box), "orientation": -1.0},
        {"center": mp.Vector3(r_rotor, +sy_box / 2.0, z_box_center), "size": mp.Vector3(sx_box, 0.0, sz_box), "orientation": +1.0},
        {"center": mp.Vector3(r_rotor, 0.0, z_bot), "size": mp.Vector3(sx_box, sy_box, 0.0), "orientation": -1.0},
        {"center": mp.Vector3(r_rotor, 0.0, z_top), "size": mp.Vector3(sx_box, sy_box, 0.0), "orientation": +1.0}
    ]
    ...
    # Stator aperture angular coordinates
    theta_rad = np.radians(theta_deg)
    xa1 = +r_rotor * np.cos(theta_rad)
    ya1 = +r_rotor * np.sin(theta_rad)
    xa2 = -r_rotor * np.cos(theta_rad)
    ya2 = -r_rotor * np.sin(theta_rad)
    ...
    def run_one_config(cfg_name):
        ...
        for task_idx in range(num_tasks):
            ...
            # 2. Walltime guard check (incorporates average moment duration safety buffer)
            ...
            # 1. Stator Membrane and Apertures (present ONLY in 'both' configuration)
            if cfg_name == "both":
                # Continuous metallic membrane: z in [-t_plate, 0]
                geometry.append(mp.Block(
                    center=mp.Vector3(0.0, 0.0, -t_plate / 2.0),
                    size=mp.Vector3(L_plate, L_plate, t_plate),
                    material=mat_obj
                ))
                # Circular aperture 1 at (+r_rotor cos theta, +r_rotor sin theta)
                geometry.append(mp.Cylinder(
                    radius=W_aperture / 2.0,
                    height=t_plate + 0.001,
                    center=mp.Vector3(xa1, ya1, -t_plate / 2.0),
                    material=bg_material
                ))
                # Circular aperture 2 at (-r_rotor cos theta, -r_rotor sin theta)
                geometry.append(mp.Cylinder(
                    radius=W_aperture / 2.0,
                    height=t_plate + 0.001,
                    center=mp.Vector3(xa2, ya2, -t_plate / 2.0),
                    material=bg_material
                ))

            # 2. Two-Needle Rotor (present in BOTH 'both' and 'self' configurations)
            # Needle 1: at (+r_rotor, 0.0)
            geometry.append(mp.Block(
                center=mp.Vector3(+r_rotor, 0.0, z_needle_center),
                size=mp.Vector3(w_needle, w_needle, H_needle),
                material=mat_obj
            ))
            # Needle 2: at (-r_rotor, 0.0)
            geometry.append(mp.Block(
                center=mp.Vector3(-r_rotor, 0.0, z_needle_center),
                size=mp.Vector3(w_needle, w_needle, H_needle),
                material=mat_obj
...
def main():
    parser = argparse.ArgumentParser(description="Rotary Vacuum Casimir Clutch Simulation Engine (Levin-Johnson Mechanism)")
    ...
    parser.add_argument("--r-rotor", type=float, default=0.35, help="Rotor needle and aperture radial offset (um)")
    parser.add_argument("--W-aperture", type=float, default=0.35, help="Stator aperture diameter (um)")
    ...
    parser.add_argument("--w-needle", type=float, default=0.04, help="Needle width (um)")
    ...
    chk_tag = f"rotary_clutch_th_{args.theta:.1f}_r_{args.r_rotor:.3f}_W_{args.W_aperture:.3f}_ztip_{args.z_tip:.4f}_res_{args.res}"

    global_rank = int(os.environ.get("SLURM_PROCID", 0))
    is_g0 = (global_rank == 0)
    if is_g0:
        print("=" * 80)
        print("ROTARY VACUUM CASIMIR CLUTCH SOLVER (Levin-Johnson Mechanism)")
        print(f"Task ID: {args.task_id} | Rotation Angle theta: {args.theta:.1f} deg")
        print(f"Rotor Radius r: {args.r_rotor*1e3:.1f} nm, Aperture Diameter W: {args.W_aperture*1e3:.1f} nm")
        ...
        print(f"Aligned Aperture Engagement: {'YES (theta = 0, Repulsive)' if abs(args.theta) < 1e-3 else 'ANGLED (Transitional/Attractive)'}")
        print("=" * 80, flush=True)

    f1_both, f1_self, f1_net = run_rotary_casimir_clutch_simulation(
        theta_deg=args.theta,
        r_rotor=args.r_rotor,
    ...
    if is_g0 and args.config == "all":
        # Total force on the 2-needle rotor: F_total = 2 * F_1
        f_total_meep = 2.0 * f1_net
        f_total_N = f_total_meep * MEEP_FORCE_TO_SI
        f_total_fN = f_total_N * 1e15

        # Normal pressure on combined needle cross-section A_rotor = 2 * w_needle^2
        # P = F_total / (2 * w^2) = F_1 / w^2
        pressure_Pa = (f1_net / (args.w_needle ** 2)) * MEEP_TO_PA

        result_data = {
            "task_id": args.task_id,
            "architecture": "rotary_casimir_clutch",
            "theta_deg": float(args.theta),
            "r_rotor_nm": float(args.r_rotor * 1e3),
            "W_aperture_nm": float(args.W_aperture * 1e3),
            "t_plate_nm": float(args.t_plate * 1e3),
            "H_needle_nm": float(args.H_needle * 1e3),
            "w_needle_nm": float(args.w_needle * 1e3),
        ...
        out_dest = f"results_rotary_clutch/task_{args.task_id:03d}_th_{int(args.theta)}deg.json"
        with open(out_dest, "w") as f:
            json.dump(result_data, f, indent=4)

        print("\n" + "=" * 80)
        print("ROTARY CASIMIR CLUTCH SIMULATION COMPLETE")
        print(f"Rotation Angle theta: {args.theta:.1f} deg")
        print(f"Single-Needle Net Force (MEEP): {f1_net:+.6e}")
        print(f"Total Rotor Force F_z (MEEP):   {f_total_meep:+.6e}")
        print(f"Total Rotor Force F_z (fN):     {f_total_fN:+.4f} fN  -->  {'[+++ CASIMIR REPULSION (LEVITATING) +++]' if f_total_meep > 0 else '[- CASIMIR ATTRACTION (ENGAGED) -]'}")
        print(f"Needle Pressure P:              {pressure_Pa:+.4e} Pa")
