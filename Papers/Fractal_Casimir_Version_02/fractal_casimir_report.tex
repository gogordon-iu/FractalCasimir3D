...
Using the high-resolution ($R=40$) data points ($L \in [0.3, 0.4, 0.5, 0.6, 2.0, 3.0, 4.0, 4.5]\ \mu\text{m}$), we perform a least-squares curve fit to compare two competing physical models:
\begin{itemize}
...
\begin{itemize}
    \item \textbf{Model 1}: $A = -0.641074$, $\alpha = 0.652606$, $B = +0.241054$.
    \item \textbf{Model 2}: $A = -0.357368$, $\alpha = 1.016235$.
\end{itemize}
...
\midrule
0.30 & -1.124345 & +0.041108 & +0.090394 \\
0.40 & -0.980524 & -0.055826 & -0.073715 \\
0.50 & -0.789350 & -0.022632 & -0.066526 \\
0.60 & -0.635006 & +0.018662 & -0.034433 \\
2.00 & -0.118022 & +0.048731 & +0.058662 \\
3.00 & -0.057953 & +0.013987 & +0.059064 \\
4.00 & -0.034166 & -0.015802 & +0.053188 \\
4.50 & -0.027400 & -0.028229 & +0.050099 \\
\midrule
\textbf{SSR} & --- & \textbf{0.00928379} & \textbf{0.03148498} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Zero-Crossing and Transition to Repulsion}
The statistical comparison in Table~\ref{tab:fit_comparison} shows that \textbf{Model 1 has a 3.39 times lower SSR than Model 2}. Model 2 significantly overestimates the attractive Casimir pressure at larger sizes, whereas the actual attraction decays much faster, on track with Model 1.

The positive asymptotic offset $B = +0.241054$ indicates that the attractive force is counterbalanced by a repulsive contribution that outscales it at larger dimensions. Setting $P(L) = 0$ in Eq.~\eqref{eq:model1}:
\begin{equation}
-\frac{0.641074}{L^{0.652606}} + 0.241054 = 0 \implies L_{\rm crossover} = \left( \frac{0.641074}{0.241054} \right)^{\frac{1}{0.652606}} \approx \mathbf{4.49\ \mu\text{m}}.
\end{equation}
This indicates a zero-crossing crossover from attractive (negative) to repulsive (positive) Casimir pressure at $L \approx 4.49\ \mu\text{m}$. For plate widths wider than $4.49\ \mu\text{m}$, the normal force becomes repulsive. At $L = 4.50\ \mu\text{m}$, the system is situated directly on the threshold of repulsion, showing a very small remaining attraction ($P \approx -0.027$).
